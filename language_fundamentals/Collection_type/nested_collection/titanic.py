...
suvived_passengers = [di for di in titanic_data if di.get("survived") == 1]
print(f"Count of survived passengers:{len(suvived_passengers)}")

# q2 : display unique passenger class 

unique_passenger_class = {di.get("class") for di in titanic_data}
print("Unique passenger class:",unique_passenger_class)

# q3 number of female passengers

count_femal_passengers = len([di for di in titanic_data if di.get("sex") == "female"])
print(f"Number of female passengers:{count_femal_passengers}")

# q4: number of survived childs 
survived_child = len([di for di in titanic_data if di.get("survived") == 1 and di.get("age") <= 12])
print(f"Count of survived child:{survived_child}")

# q5: name whose fare > 30

name_whose_fare_gt_30 = [di.get("name") for di in titanic_data if di.get("fare") > 30]
print("Name whose fare > 30:",name_whose_fare_gt_30)

# q6: number survived female
survived_female_count = len([di for di in titanic_data if di.get("sex") == "female" and di.get("survived") == 1])
print("Number of survived female:",survived_female_count)
...
print("Name of survived people:",survived_people_name)

# q9:survived classes
survived_classes = {di.get("class") for di in titanic_data if di.get("survived") == 1}
print("Classes of people survived:",survived_classes)

# q10:female survival rate

female_survival_rate = (survived_female_count/count_femal_passengers)*100
